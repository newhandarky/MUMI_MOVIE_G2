	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("update".equals(action)) { // 來自update_emp_input.jsp的請求
			...
			try {
				...
				/*************************** 2.開始修改資料 *****************************************/
				EmployeeService employeeSvc = new EmployeeService();
				employeeVO = employeeSvc.updateEmp(emp_id, emp_account, emp_name, emp_password, emp_nickname, emp_phone, emp_hiredate);
		...
		if ("insert".equals(action)) { // 來自addEmp.jsp的請求
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					req.setAttribute("employeeVO", employeeVO); // 含有輸入格式錯誤的empVO物件,也存入req
					RequestDispatcher failureView = req.getRequestDispatcher("/view/employee/employee_register.jsp");
					failureView.forward(req, res);
					return;
				}
				...
				/*************************** 2.開始新增資料 ***************************************/
				EmployeeService employeeSvc = new EmployeeService();
				employeeVO = employeeSvc.addEmp(emp_account, emp_name, emp_password, emp_nickname, emp_phone, emp_hiredate);
