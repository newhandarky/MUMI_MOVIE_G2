	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("login".equals(action)) { // 來自login.jsp的請求
			...
			try {
				...
				Cookie cookie = new Cookie("nickname", memVO.getMem_nickname());

		
				cookie.setPath("/");
				// 如果cookie的值中含有中文時，需要對cookie進行編碼，不然會產生亂碼
				try {
					URLEncoder.encode(memVO.getMem_nickname(), "utf-8");
				...
				res.sendRedirect(req.getContextPath() + "/view/index/index.jsp"); // *工作3:
