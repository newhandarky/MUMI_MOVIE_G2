...
<body>
			...
			<c:forEach var="articleVO" items="${list}">
				...
				<tbody>
					<tr>
						...
						<td scope="col"><img
							src="ShowPicServlet?article_id=${articleVO.article_id}"
							width="120"></td>
						...
						<td scope="col"><fmt:formatDate
								value="${articleVO.article_publish}"
								pattern="yyyy-MM-dd HH:mm:ss" /></td>
						<td scope="col"><fmt:formatDate
								value="${articleVO.article_updated}"
								pattern="yyyy-MM-dd HH:mm:ss" /></td>
						<td scope="col">${articleVO.emp_id}</td>
						<td scope="col">${articleVO.article_state}</td>
						<td scope="col">${articleVO.re_article_id}</td>
						<td scope="col">
							<button type="button" class="btn_contain">內文</button>
						</td>
						<td scope="col">
							<form method="post"
								action="<%=request.getContextPath()%>/view/forum_article/forumArticle.do">
	...
	<script
		src="<%=request.getContextPath()%>/view/forum_article/js/listAllArticle.js"></script>
</body>
</html>